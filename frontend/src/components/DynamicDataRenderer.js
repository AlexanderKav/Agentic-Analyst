...
class DynamicDataRenderer {
      ...
      if (typeof value === 'object' && value !== null) {
        ...
        for (const [period, description] of Object.entries(value)) {
          if (typeof description === 'string' && description.length > 0) {
            ...
            const descStr = Object.entries(description)
              .map(([k, v]) => `${k}: ${safeStringify(v)}`)
              .join(', ');
            anomalyText += `• ${productName} in ${period}: {${descStr}}\n`;
          }
        }
        ...
        const productName = key.replace(/_/g, ' ').replace('Plan', ' Plan');
        const descStr = Object.entries(value)
          .map(([k, v]) => `${k}: ${safeStringify(v)}`)
          .join(', ');
        anomalyText += `• ${productName}: {${descStr}}\n`;
      }
    }
  ...
  static renderPrimitive(data, title) {
    return (
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        <Typography variant="body1">{safeStringify(data)}</Typography>
      </Paper>
    );
  }
            ...
            {data.map((row, idx) => (
              <TableRow key={idx}>
                {Object.values(row).map((value, i) => (
                  <TableCell key={i}>
                    {typeof value === 'number' ? this.formatCurrency(value) : safeStringify(value)}
                  </TableCell>
                ))}
              </TableRow>
            ))}
    ...
    if (data.key_findings || data.relevant_trends || data.additional_context) {
      ...
      if (data.key_findings && Array.isArray(data.key_findings)) {
        ...
        data.key_findings.forEach((finding, idx) => {
          content.push(`• ${safeStringify(finding)}`);
        });
        content.push("");
      ...
      if (data.relevant_trends && Array.isArray(data.relevant_trends)) {
        ...
        data.relevant_trends.forEach((trend, idx) => {
          content.push(`• ${safeStringify(trend)}`);
        });
        content.push("");
      ...
      if (data.additional_context) {
        content.push(`**Context:** ${safeStringify(data.additional_context)}`);
      }
    ...
    return (
      <Paper sx={{ p: 2, mb: 2 }}>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
          {data.map((item, idx) => (
            <Chip key={idx} label={typeof item === 'number' ? this.formatCurrency(item) : safeStringify(item)} />
          ))}
        </Box>
      </Paper>
    );
                    ...
                    {value.map((item, idx) => (
                      <li key={idx}>
                        <Typography variant="body2">
                          {typeof item === 'object' ? JSON.stringify(item) : safeStringify(item)}
                        </Typography>
                      </li>
                    ))}
                  </Box>
    ...
    if (data.identified) {
      ...
      if (Array.isArray(data.identified)) {
        ...
        data.identified.forEach((anomaly, idx) => {
          content.push(`• ${safeStringify(anomaly)}`);
        });
      ...
      if (data.severity) {
        content.push("");
        content.push(`**Severity:** ${safeStringify(data.severity)}`);
      }
  ...
  // Check for next_steps array (preferred format)
  if (data.next_steps && Array.isArray(data.next_steps)) {
    let content = ["**Recommended Next Steps:**"];
    ...
    if (content.length === 1) return null; // No valid recommendations
  ...
  // Check for direct recommendations array
  ...
  static renderGenericTable(data, title) {
          ...
          <TableBody>
            {data.slice(0, 10).map((row, idx) => (
              <TableRow key={idx}>
                {keys.map(key => (
                  <TableCell key={key}>
                    {typeof row[key] === 'number' ? this.formatCurrency(row[key]) : safeStringify(row[key])}
                  </TableCell>
                ))}
              </TableRow>
            ))}
